        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div>
            <div className="flex items-center gap-2.5 mb-1">
              <h1 className="text-xl sm:text-2xl font-extrabold text-gray-900">Support</h1>
              {isPro ? (
          ...
          <div className="flex bg-gray-100 rounded-xl p-1 gap-1 self-start">
            <button onClick={() => setView("form")}
              className={`px-3 sm:px-4 py-2 rounded-lg text-sm font-semibold transition-all ${view === "form" ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700"}`}>
              New Ticket
            </button>
            <button onClick={() => setView("tickets")}
              className={`relative px-3 sm:px-4 py-2 rounded-lg text-sm font-semibold transition-all ${view === "tickets" ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700"}`}>
              My Tickets
              {tickets.length > 0 && (
              ...
              {tickets.map((ticket) => {
                ...
                return (
                  <div key={ticket.id}
                    ...
                    {/* Top bar */}
                    <div className="flex items-center justify-between px-3 sm:px-5 py-3 border-b border-gray-50 gap-2">
                      <div className="flex items-center gap-1.5 sm:gap-2.5 flex-wrap min-w-0">
                        ...
                        <span className="text-[10px] text-gray-400 hidden sm:inline">#{ticket.id.slice(-6)}</span>
                    ...
                    <div className="px-3 sm:px-5 py-4">
                      <div className="flex items-start justify-between gap-2 sm:gap-3 mb-3">
                        <div className="min-w-0">
                          <h3 className="font-bold text-gray-900 text-sm sm:text-base mb-0.5 break-words">{ticket.subject}</h3>
                          <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">{ticket.category}</span>
                        </div>
                        {hasAdminReply && (
                          <div className="flex items-center gap-1 sm:gap-1.5 bg-blue-50 border border-blue-100 text-blue-700 text-[10px] sm:text-xs font-bold px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl flex-shrink-0">
                            <svg className="w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
                            </svg>
                            <span className="hidden sm:inline">Admin replied</span>
...
function ConversationModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center sm:p-4" onClick={onClose}>
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-2xl h-[92vh] sm:h-auto sm:max-h-[88vh] flex flex-col overflow-hidden"
        ...
        <div className="px-3 sm:px-5 py-3 sm:py-4 border-b border-gray-100 flex-shrink-0">
          <div className="flex items-start justify-between gap-2 sm:gap-3">
            <div className="min-w-0">
              ...
              <h2 className="font-bold text-gray-900 text-sm sm:text-base leading-snug break-words">{ticket.subject}</h2>
              <p className="text-[10px] sm:text-[11px] text-gray-400 mt-0.5">#{ticket.id.slice(-8)} · Opened {formatFull(ticket.createdAt)}</p>
            </div>
        ...
        {/* Reply */}
        <div className="flex-shrink-0 px-3 sm:px-5 py-3 sm:py-4 border-t border-gray-100 bg-white">
          {canReply ? (
            <>
