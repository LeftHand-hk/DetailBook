...
export default function BookingsPage() {
  ...
  const assignStaff = async (bookingId: string, staffId: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        const data = await response.json().catch(() => ({}));
        throw new Error(data.error || "Could not assign staff");
      }
    } catch (error) {
      setActionError(error instanceof Error ? error.message : "Could not assign staff");
      return;
    }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      {/* Filters + Search — compact toolbar */}
      <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-5">
        {/* Status segmented control */}
        <div className="inline-flex bg-gray-100 rounded-lg p-0.5 overflow-x-auto scrollbar-none -mx-4 px-4 sm:mx-0 sm:px-0">
          {(["all", "upcoming", "completed", "pending", "cancelled"] as FilterType[]).map((f) => {
            ...
            return (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold capitalize transition-colors ${
                  active
              ...
              >
                {f}
                <span className={`text-[10px] leading-none px-1.5 py-0.5 rounded-md tabular-nums ${
                  active ? "bg-blue-100 text-blue-700" : "bg-white text-gray-500"
                }`}>{counts[f]}</span>
              </button>
            );
          })}
        ...
        {/* Date range as a select to reclaim horizontal space */}
        <div className="relative">
          <select
            value={range}
            onChange={(e) => setRange(e.target.value as RangeType)}
            className="appearance-none bg-white border border-gray-200 rounded-lg pl-3 pr-8 py-1.5 text-xs font-semibold text-gray-700 hover:border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 cursor-pointer"
          >
        ...
        {/* Search — fills the rest of the row on desktop */}
        <div className="relative sm:ml-auto sm:flex-1 sm:max-w-xs">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 bg-white border border-gray-200 rounded-lg text-xs text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-gray-300"
          />
        </div>
      </div>
            ...
            <div className="bg-white w-full sm:max-w-md max-h-[92vh] sm:max-h-none sm:h-full rounded-t-3xl sm:rounded-none shadow-2xl animate-slideUp sm:animate-slideInRight flex flex-col"
              ...
              <div className="bg-white border-b border-gray-100 px-5 py-3 flex items-center justify-between flex-shrink-0">
                ...
                <button onClick={() => setSelected(null)}
                  className="inline-flex items-center gap-1.5 px-2.5 py-2 rounded-xl text-sm font-semibold text-gray-600 hover:bg-gray-100 active:bg-gray-200 transition-colors -mr-2">
              ...
              <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
                {actionError && (
                  <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-xl px-3 py-2">{actionError}</div>
                )}
